<template>
	<view class="verify iphone-safe-area">
		<block v-if="scan_code_open == false">
			<view class="scan" >
		...
		<mumu-get-qrcode @success='qrcodeSucess' :continue='false' @error="qrcodeError" v-if="scan_code_open"></mumu-get-qrcode>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		qrcodeSucess(data) {
			this.scan_code_open = false;
			var code = this.getQueryVariable(data,'code');
			this.verify_code = code;
			this.search();
		},
		qrcodeError(err) {
