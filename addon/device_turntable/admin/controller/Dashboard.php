        ...
        if (request()->isAjax()) {
            try {
                ...
                return json(success(0, '', [
                    'boards' => intval($boards), 'devices' => intval($devices), 'slots' => intval($slots),
                    'records_total' => intval($records_total), 'records_today' => intval($records_today),
                    'profit_total' => floatval($profit_total), 'profit_today' => floatval($profit_today),
                    'verified_today' => $verified_today, 'settled_today' => $settled_today,
                    'role_dist' => $role_dist,
                ]));
            } catch (\Throwable $e) {
                return json(success(0, '', [
                    'boards' => 0, 'devices' => 0, 'slots' => 0,
                    'records_total' => 0, 'records_today' => 0,
                    'profit_total' => 0, 'profit_today' => 0,
                    'verified_today' => 0, 'settled_today' => 0,
                    'error' => 'dashboard_stats_error', 'message' => $e->getMessage(),
                ]));
            }
        } else {
