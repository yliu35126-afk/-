...
class Board extends BaseAdmin
{
    ...
    public function add()
    {
        if (request()->isAjax()) {
            ...
            if ($name === '') return success(-1, '名称不能为空', null);
            $data = [
            ...
            $res = model('lottery_board')->add($data);
            if ($res) return success(0, '添加成功', ['board_id' => $res]);
    ...
    public function edit()
    {
        ...
        $model = model('lottery_board');
        if (request()->isAjax()) {
            if ($board_id <= 0) return success(-1, '参数错误', null);
            $name = input('name', '');
            $status = (int)input('status', 1);
            $round_control = (int)input('round_control', 0);
            if ($name === '') return success(-1, '名称不能为空', null);
    ...
    public function delete()
    {
        if (request()->isAjax()) {
            $board_id = (int)input('board_id', 0);
            if ($board_id <= 0) return success(-1, '参数错误', null);
            $res = model('lottery_board')->delete([['board_id', '=', $board_id], ['site_id', '=', $this->site_id]]);
