...
layui.use('form', function () {
	...
	form.verify({
		...
		phone: function(value) {
			if (value != '') {
				if (!ns.getRegexp('phone').test(value)) {
					return '请输入正确的电话号码!';
				}
			}
		},
		mobile_or_phone : function(value, item) {
			if (value != '') {
				if (!(ns.getRegexp('mobile').test(value)) && !(ns.getRegexp('phone').test(value))) {
		...
		bankcard: function(value) {
			if (value != '') {
				if (!ns.getRegexp('bankcard').test(value)) {
					return '请输入正确的银行卡号!';
				}
			}
		}
...
function Upload(options) {
    ...
    options.post = options.post || ns.route[0];
	//插件特殊处理
    if (['store','supply'].indexOf(options.post) > -1){
