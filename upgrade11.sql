...
-- 轮次控制：为抽奖盘与格子补充轮次字段（若不存在）
ALTER TABLE `lottery_board`
  ADD COLUMN IF NOT EXISTS `round_no` INT(11) NOT NULL DEFAULT 0 COMMENT '当前轮次编号' AFTER `status`,
  ADD COLUMN IF NOT EXISTS `auto_reset_round` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '候选为空自动下一轮' AFTER `round_no`;

ALTER TABLE `lottery_slot`
  ADD COLUMN IF NOT EXISTS `round_qty` INT(11) NOT NULL DEFAULT 0 COMMENT '本轮可中次数' AFTER `weight`;
