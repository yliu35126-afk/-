...
class Shop extends BaseAdmin
{
    ...
    public function lists()
    {
        if (request()->isAjax()) {
            ...
            if(!empty($shop_list['data']['list']))
            {
                $cert_id_array = [];
                foreach ($shop_list[ 'data' ][ 'list' ] as $k => $v) {

                    if ($v[ 'cert_id' ] > 0) {
                        $cert_id_array[] =  $v[ 'cert_id' ];
                    }
                }
                $cert_list = $shop_model->getShopCertListInShop([['cert_id', 'in', $cert_id_array]]);
                foreach ($shop_list[ 'data' ][ 'list' ] as $k => $v) {
                    foreach ($cert_list['data'] as $k_cert => $v_cert)
                    {
                        if($v['cert_id'] == $v_cert['cert_id'])
                        {
                            $shop_list[ 'data' ][ 'list' ][ $k ][ 'cert_info' ] = $v_cert;
                        }
                    }
                }
            }
