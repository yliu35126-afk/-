...
var positionHtml = '<div class="layui-form-item">';
	positionHtml +=	 '<label class="layui-form-label sm">{{data.label}}</label>';
	positionHtml +=	 '<div class="layui-input-block">';
	positionHtml +=		 '<template v-for="(item,index) in list" v-bind:k="index">';
	positionHtml +=			 '<div v-on:click="parent[data.field]=item.value" v-bind:class="{ \'layui-unselect layui-form-radio\' : true,\'layui-form-radioed\' : (parent[data.field]==item.value) }"><i class="layui-anim layui-icon">&#xe643;</i><div>{{item.label}}</div></div>';
	positionHtml +=		 '</template>';
	positionHtml +=	 '</div>';
	positionHtml += '</div>';

Vue.component("search-position", {
	props: {
		data: {
			type: Object,
			default: function () {
	...
	created: function () {
		if(!this.$parent.data.verify) this.$parent.data.verify = [];
		this.$parent.data.verify.push(this.verify);//加载验证方法
		if (this.data.label == undefined) this.data.label = "显示位置";
		if (this.data.field == undefined) this.data.field = "borderType";
	...
	watch: {
		data: function (val, oldVal) {
			if (val.field == undefined) val.field = oldVal.field;
			if (val.label == undefined) val.label = "显示位置";
		},
	},
	methods: {
		verify : function () {
...
typeHtml +=		 			`<i :class="['iconfont',item.icon_img]"></i>`;
typeHtml +=		 		'</div>';
typeHtml +=			 '</template>';
typeHtml +=			 '</div>';
...
typeHtml +=	 		'</div>';
typeHtml +=	 	'</div>';
typeHtml +=		'<div v-if="parent[data.field] == 3" class="ns-word-aux" style="margin-left: 14px;margin-top:5px;">安装站内信插件后显示消息图标</div>';
