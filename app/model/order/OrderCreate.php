...
//        }
    ...
    public function itemPayment($shop_item, $data, $is_coupon = true, $self = null)
    {
        ...
        $config_model = new ExpressConfig();
        $deliver_type = $config_model->getDeliverTypeSort($shop_item['site_id']);
        $express_deliver_type = [];
        if(!empty($deliver_type) && $deliver_type[ 'data' ][ 'value' ]){
            $express_deliver_type = explode(',', $deliver_type[ 'data' ][ 'value' ][ 'deliver_type' ]);
        }
        ...
        if ($self->is_virtual == 0) {
            if (!empty($data['member_address'])) {
                ...
                if ($shop_item["express_config"]["is_use"] == 1) {
                    if($express_deliver_type){
                        foreach ($express_deliver_type as $key => $val){
                            if($val == "express"){
                                $express_type[] = ["title" => Express::express_type["express"]["title"], "name" => "express", "sort" => $key];
                            }
                        }
                    }else{
                ...
                if ($shop_item["store_config"]["is_use"] == 1) {
                    ...
                    if($express_deliver_type){
                        foreach ($express_deliver_type as $key => $val){
                            if($val == "store"){
                                $express_type[] = ["title" => Express::express_type["store"]["title"], "name" => "store", "store_list" => $store_list, 'store_id' => $store_id, "sort" => $key];
                            }
                        }
                    }else{
                ...
                if ($shop_item["local_config"]["is_use"] == 1) {
                    ...
                    if($express_deliver_type){
                        foreach ($express_deliver_type as $key => $val){
                            if($val == "local"){
                                $express_type[] = ["title" => "外卖配送", "name" => "local", "sort" => $key];
                            }
                        }
                    }else{
        ...
        if($express_deliver_type){
            $last_ages = array_column ($express_type , 'sort') ;
            array_multisort($last_ages ,SORT_ASC ,$express_type) ;
        }
