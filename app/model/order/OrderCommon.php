...
//                    }
    ...
    public function getMemberOrderPageList($condition = [], $page = 1, $page_size = PAGE_LIST_ROWS, $order = '', $field = '*')
    {
        ...
        if (!empty($order_list[ 'list' ])) {
            ...
            $order_goods_list = model("order_goods")->getList([['order_id', 'in', $order_id_array]]);
            foreach ($order_list[ 'list' ] as $k => $v) {
                foreach ($order_goods_list as $k_order_goods => $v_order_goods)
                {
                    if($v['order_id'] == $v_order_goods['order_id'])
                    {
                        $order_list[ 'list' ][ $k ][ 'order_goods' ][] = $v_order_goods;
                    }
                }
            }
        }
