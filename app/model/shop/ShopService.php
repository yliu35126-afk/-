...
class ShopService extends BaseModel
{
	...
	public function serviceApplyList($site_id)
	{
        ...
        $shop_service_list = model('shop_service')->getList([['site_id', '=', $site_id]],'*','apply_id desc');
		//通过全局配置查询对应的服务信息
		foreach ($service_list_arr as $key => $value) {
		    $service_key = $value['key'];
		    $service_list_arr[$key]['status'] = $shop_service_info[$service_key];
		    foreach ($shop_service_list as $k_shop_service => $shop_service)
            {
                if(($shop_service['service_type'] == $value['key'])&&(!isset($service_list_arr[$key]['apply_status']))){
                    // 查询到对应的信息,进行审核状态的获取
                    $service_list_arr[$key]['apply_status']  =  $shop_service['status'];
                    $service_list_arr[$key]['remark']  =  $shop_service['remark'];
                }
            }
		    if(!isset($service_list_arr[$key]['apply_status'])){
                $service_list_arr[$key]['apply_status'] = 0;
