...
class Shop extends BaseModel
{
     ...
     * @return array
     */
    public function getShopGroup($site_id)
    {
        $cache = Cache::get("shop_cache_shop_group_getGroup_" . $site_id);
        if (!empty($cache)) {
            return $this->success($cache);
        }
        $shop_group = model("shop")->getInfo([['site_id', '=', $site_id]], 'group_id');
        Cache::tag("shop")->set("shop_cache_shop_group_getGroup_" . $site_id, $shop_group);
        return $this->success($shop_group);
    }
    /**
     * 获取店铺信息
...
//		客服配置信息
     ...
     * @return array
     */
    public function getShopCertListInShop($condition, $field = '*')
    {
        $res = model('shop_cert')->getList($condition, $field);
        return $this->success($res);
    }
